...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Sending Google Ads PDF report to ayodeleheebrahymn@outlook.com via SMTP (${smtpHost})...`);
    const info = await transporter.sendMail({
      from: `"Carbon365 Analytics" <${smtpUser}>`,
      to: "ayodeleheebrahymn@outlook.com",
      subject: `Google Ads Performance Report — ${targetDateStr}`,
      text: `Hello,\n\nPlease find attached the daily Google Ads Performance Report for ${targetDateStr}.\n\nBest regards,\nCarbon Car Care Team`,
      attachments: [
        {
