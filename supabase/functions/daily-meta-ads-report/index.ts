...
serve(async (req) => {
  ...
  try {
    ...
    // Filter to strictly allowed campaigns if available
    ...
    console.log(`Sending Meta Ads PDF report to ayodeleheebrahymn@outlook.com via SMTP (${smtpHost})...`);
    const info = await transporter.sendMail({
      from: `"Carbon365 Analytics" <${smtpUser}>`,
      to: "ayodeleheebrahymn@outlook.com",
      subject: `Meta Ads Performance Report — ${targetDateStr}`,
      text: `Hello,\n\nPlease find attached the daily Meta Ads Performance Report for ${targetDateStr}.\n\nBest regards,\nCarbon Car Care Team`,
      attachments: [
        {
