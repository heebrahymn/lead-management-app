...
serve(async (req) => {
  ...
  try {
    ...
    if (metricsData) {
      metricsData.forEach(row => {
        ...
        if (!campaignStats[row.campaign_name]) {
          campaignStats[row.campaign_name] = { name: row.campaign_name, spend: 0, clicks: 0, impressions: 0, whatsapp_clicks: 0, reach: 0 };
        }
    ...
    // Filter PDF report campaigns strictly to requested allowed list
    const matchedForPdf = sortedCampaigns.filter(c =>
      ALLOWED_CAMPAIGN_NAMES.some(allowed => c.name.trim().toLowerCase() === allowed || c.name.trim().toLowerCase().includes(allowed))
    );
    const pdfCampaigns = matchedForPdf.length > 0 ? matchedForPdf : sortedCampaigns;

    // Generate PDF Buffer
    const pdfBuffer = generatePdfReport(targetDatePdfStr, pdfCampaigns);
    ...
    console.log(`Sending Meta Ads email report with PDF attachment via SMTP (${smtpHost})...`);
    const info = await transporter.sendMail({
      from: `"Carbon365 Analytics" <${smtpUser}>`,
      to: recipients,
      subject: `Meta Ads Analytics Report, ${targetDateStrFormatted}`,
      html: htmlContent,
      attachments: [
        {
    ...
    console.log("Meta Ads Email + PDF sent successfully: ", info.messageId);
